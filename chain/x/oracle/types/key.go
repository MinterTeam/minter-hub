...
func GetClaimKey(details Claim) []byte {
	...
	key := make([]byte, len(OracleClaimKey)+claimTypeLen+sdk.AddrLen+len(epochBz)+len(detailsHash))
	...
	copy(key[len(OracleClaimKey)+claimTypeLen+sdk.AddrLen:], epochBz)
	copy(key[len(OracleClaimKey)+claimTypeLen+sdk.AddrLen+len(epochBz):], detailsHash)
	return key
}
