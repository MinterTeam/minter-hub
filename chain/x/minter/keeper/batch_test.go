...
func TestBatches(t *testing.T) {
	...
	allVouchers := sdk.Coins{types.NewMinterCoin(99999, myTokenContractAddr).PeggyCoin()}
	...
	for i, v := range []uint64{2, 3, 2, 1} {
		amount := types.NewMinterCoin(uint64(i+100), myTokenContractAddr).PeggyCoin()
		fee := types.NewMinterCoin(v, myTokenContractAddr).PeggyCoin()
	...
	expFirstBatch := &types.OutgoingTxBatch{
		...
		Transactions: []*types.OutgoingTransferTx{
			{
				Id:          2,
				Erc20Fee:    types.NewMinterCoin(3, myTokenContractAddr),
				Sender:      mySender.String(),
				DestAddress: myReceiver,
				Erc20Token:  types.NewMinterCoin(101, myTokenContractAddr),
			},
			{
				Id:          1,
				Erc20Fee:    types.NewMinterCoin(2, myTokenContractAddr),
				Sender:      mySender.String(),
				DestAddress: myReceiver,
				Erc20Token:  types.NewMinterCoin(100, myTokenContractAddr),
			},
		},
	...
	expUnbatchedTx := []*types.OutgoingTx{
		{
			BridgeFee: types.NewMinterCoin(2, myTokenContractAddr).PeggyCoin(),
			Sender:    mySender.String(),
			DestAddr:  myReceiver,
			Amount:    types.NewMinterCoin(102, myTokenContractAddr).PeggyCoin(),
		},
		{
			BridgeFee: types.NewMinterCoin(1, myTokenContractAddr).PeggyCoin(),
			Sender:    mySender.String(),
			DestAddr:  myReceiver,
			Amount:    types.NewMinterCoin(103, myTokenContractAddr).PeggyCoin(),
		},
	}
	...
	expSecondBatch := &types.OutgoingTxBatch{
		...
		Transactions: []*types.OutgoingTransferTx{
			{
				Id:          6,
				Erc20Fee:    types.NewMinterCoin(5, myTokenContractAddr),
				Sender:      mySender.String(),
				DestAddress: myReceiver,
				Erc20Token:  types.NewMinterCoin(101, myTokenContractAddr),
			},
			{
				Id:          5,
				Erc20Fee:    types.NewMinterCoin(4, myTokenContractAddr),
				Sender:      mySender.String(),
				DestAddress: myReceiver,
				Erc20Token:  types.NewMinterCoin(100, myTokenContractAddr),
			},
	...
	expUnbatchedTx = []*types.OutgoingTx{
		{
			BridgeFee: types.NewMinterCoin(3, myTokenContractAddr).PeggyCoin(),
			Sender:    mySender.String(),
			DestAddr:  myReceiver,
			Amount:    types.NewMinterCoin(101, myTokenContractAddr).PeggyCoin(),
		},
		{
			BridgeFee: types.NewMinterCoin(2, myTokenContractAddr).PeggyCoin(),
			Sender:    mySender.String(),
			DestAddr:  myReceiver,
			Amount:    types.NewMinterCoin(100, myTokenContractAddr).PeggyCoin(),
		},
		{
			BridgeFee: types.NewMinterCoin(2, myTokenContractAddr).PeggyCoin(),
			Sender:    mySender.String(),
