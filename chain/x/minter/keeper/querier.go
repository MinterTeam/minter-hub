...
func NewQuerier(keeper Keeper) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err error) {
		...
		case QueryBatchConfirms:
			return queryAllBatchConfirms(ctx, path[1], keeper) // Tested (lightly)
...
func queryValsetRequest(ctx sdk.Context, path []string, keeper Keeper) ([]byte, error) {
	...
	for i := nonce; i > 0; i-- {
		valset = keeper.GetValsetRequest(ctx, i)
		if valset != nil {
			break
		}
	}
...
// When nothing found an empty json array is returned. No pagination.
func queryAllBatchConfirms(ctx sdk.Context, nonceStr string, keeper Keeper) ([]byte, error) {
	nonce, err := types.UInt64FromString(nonceStr)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, err.Error())
	}
	...
	keeper.IterateBatchConfirmByNonce(ctx, nonce, func(_ []byte, c types.MsgConfirmBatch) bool {
		confirms = append(confirms, c)
		return false
	})
	if len(confirms) == 0 {
...
func queryBatch(ctx sdk.Context, nonce string, tokenContract string, keeper Keeper) ([]byte, error) {
	...
	if types.ValidateMinterAddress(tokenContract) != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrUnknownRequest, err.Error())
	}
	foundBatch := keeper.GetOutgoingTXBatch(ctx, parsedNonce)
	if foundBatch == nil {
