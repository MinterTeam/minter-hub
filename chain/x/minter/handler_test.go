...
func TestHandleValsetRequest(t *testing.T) {
	...
	assert.Equal(t, "", valset.Members[0].MinterAddress)
}

func TestHandleMsgSendToMinter(t *testing.T) {
	var (
...
func TestHandleCreateEthereumClaimsSingleValidator(t *testing.T) {
	var (
		...
		myValAddr                         = sdk.ValAddress(myOrchestratorAddr) // revisit when proper mapping is impl in keeper
		myNonce                           = uint64(1)
		anyMinterAddr                     = "Mxf9613b532673Cc223aBa451dFA8539B87e1F666D"
		myBlockTime                       = time.Date(2020, 9, 14, 15, 20, 10, 0, time.UTC)
	)
	k, ctx, keepers := keeper.CreateTestEnv(t)
	k.StakingKeeper = keeper.NewStakingKeeperMock(myValAddr)
	...
	ethClaim := types.MsgDepositClaim{
		EventNonce:     myNonce,
		CoinId:         myMinterCoin.CoinId,
		Amount:         myMinterCoin.Amount,
		MinterSender:   anyMinterAddr,
	...
	assert.Equal(t, sdk.Coins{sdk.NewInt64Coin("minter/1", 12)}, balance)
	...
	require.Error(t, err)
	balance = keepers.BankKeeper.GetAllBalances(ctx, myCosmosAddr)
	assert.Equal(t, sdk.Coins{sdk.NewCoin("minter/1", amount)}, balance)
	...
	require.NoError(t, err)
	balance = keepers.BankKeeper.GetAllBalances(ctx, myCosmosAddr)
	assert.Equal(t, sdk.Coins{sdk.NewCoin("minter/0", amount)}, balance)
}
...
func TestHandleCreateEthereumClaimsMultiValidator(t *testing.T) {
	...
	assert.Equal(t, sdk.Coins{sdk.NewInt64Coin("minter/0", 12)}, balance2)
	...
	assert.Equal(t, sdk.Coins{sdk.NewInt64Coin("minter/0", 12)}, balance3)
}
