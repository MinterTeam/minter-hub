...
func (k Keeper) RefundOutgoingTx(ctx sdk.Context, id uint64, tx *types.OutgoingTx) {
	if err := k.removeFromUnbatchedTXIndex(ctx, tx.BridgeFee, id); err != nil {
		panic(err)
	}
	k.removePoolEntry(ctx, id)

	vouchers := sdk.Coins{tx.Amount}
	...
	if tx.RefundAddr[2:] == "Mx" {
		...
		receiver, err := sdk.AccAddressFromBech32(tx.RefundAddr)
		if err != nil {
			panic(err)
		}
		if err := k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, receiver, vouchers); err != nil {
