...
func toClaimType(input int32) ClaimType {
	if input == 1 {
		...
		return CLAIM_TYPE_SEND_TO_MINTER
	} else {
...
func fromClaimType(input ClaimType) int32 {
	if input == CLAIM_TYPE_DEPOSIT {
		...
		return 2
	} else if input == CLAIM_TYPE_SEND_TO_MINTER {
		return 3
	} else {
		return 0
	}
...
var (
	...
	_ EthereumClaim = &MsgSendToMinterClaim{}
...
func (e *MsgSendToMinterClaim) ValidateBasic() error {
	if err := ValidateMinterAddress(e.MinterReceiver); err != nil {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidAddress, e.MinterReceiver)
	}
...
// Type should return the action
func (msg MsgSendToMinterClaim) Type() string { return "send_to_minter_claim" }

// Route should return the name of the module
func (msg MsgSendToMinterClaim) Route() string { return RouterKey }

// Hash implements BridgeDeposit.Hash
func (b *MsgSendToMinterClaim) ClaimHash() []byte {
	path := fmt.Sprintf("%s/%s/%s/", b.TokenContract, string(b.EthereumSender), b.MinterReceiver)
