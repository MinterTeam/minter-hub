...
/// on the Peggy contract
#[derive(Serialize, Deserialize, Debug, Default, Clone, Eq, PartialEq, Hash)]
pub struct SendToMinterEvent {
    /// The token contract address for the deposit
    pub erc20: EthAddress,
    /// The Ethereum Sender
    pub sender: EthAddress,
    /// The Minter destination
...
impl SendToMinterEvent {
    pub fn from_log(input: &Log) -> Result<SendToMinterEvent, PeggyError> {
        let topics = (
            input.topics.get(1),
            input.topics.get(2),
            input.topics.get(3),
    ...
    pub fn from_logs(input: &[Log]) -> Result<Vec<SendToMinterEvent>, PeggyError> {
        let mut res = Vec::new();
        for item in input {
            res.push(SendToMinterEvent::from_log(item)?);
        }
        Ok(res)
    }
    /// returns all values in the array with event nonces greater
